import React from 'react';

const CommitIcon: React.FC = () => (
    <svg
        width="20"
        height="20"
        fill="grey"
        version="1.1"
        id="Layer_1"
        xmlns="http://www.w3.org/2000/svg"
        viewBox="230.4 0 25.6 512"
    >
        <g>
            <path
                d="M256,179.2v25.6c28.3,0.1,51.1,22.9,51.2,51.2c-0.1,28.3-22.9,51.1-51.2,51.2c-28.3-0.1-51.1-22.9-51.2-51.2
 c0.1-28.3,22.9-51.1,51.2-51.2V179.2v-25.6c-56.6,0-102.4,45.8-102.4,102.4c0,56.6,45.8,102.4,102.4,102.4
 c56.6,0,102.4-45.8,102.4-102.4c0-56.6-45.8-102.4-102.4-102.4V179.2z"
            />
            <path d="M230.4,25.6v153.6c0,14.1,11.5,25.6,25.6,25.6s25.6-11.5,25.6-25.6V25.6C281.6,11.5,270.1,0,256,0S230.4,11.5,230.4,25.6" />
            <path
                d="M230.4,332.8v153.6c0,14.1,11.5,25.6,25.6,25.6s25.6-11.5,25.6-25.6V332.8c0-14.1-11.5-25.6-25.6-25.6
 S230.4,318.7,230.4,332.8"
            />
        </g>
    </svg>
);

export default CommitIcon;
